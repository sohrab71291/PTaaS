...
const TEST_NAME = __ENV.TEST_NAME || 'Archer Record Session Replay';
...
const SCENARIO_NAME = __ENV.SCENARIO_NAME || 'archer_record_replay';
...
export const options = {
  ...
  thresholds: {
    'http_req_duration{endpoint_type:app}': ['p(95)<5000'],
    'http_req_failed{endpoint_type:app}': ['rate<10.01'],
    'checks': ['rate>0.9'],
  },
};
...
function isEffectivelyExpectedResponse(reqDef, response) {
  if (isKnownBenignNotFound(reqDef, response)) return true;
  if (isIdempotentDeleteOutcome(reqDef, response)) return true;
  if (isResponseStatusExpected(response, reqDef.expectedStatus)) return true;
...
function replayStep(reqDef, jar, correlationVars) {
  ...
  function doRequest(authHeaders) {
    ...
    const params = {
      ...
      jar: jar,
      responseCallback: function (res) { return isEffectivelyExpectedResponse(reqDef, res); },
    };
  ...
  check(res, {
    ...
    [reqDef.name + ' response time < 5000ms']: function (r) { return r.timings.duration < 5000; },
