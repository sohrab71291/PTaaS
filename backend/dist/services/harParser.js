...
exports.CORRELATION_PLACEHOLDER_RE = void 0;
exports.applyIdCorrelation = applyIdCorrelation;
...
function collectIdCandidates(body, prefix = '', depth = 0, out = []) {
    if (body == null || depth > 3 || out.length > 25 || typeof body !== 'object')
        return out;
    if (Array.isArray(body)) {
        if (body.length > 0)
            collectIdCandidates(body[0], prefix ? `${prefix}.0` : '0', depth + 1, out);
        return out;
    }
...
function substituteLiteral(text, literal, replacement) {
...
function tokenNameFromPath(jsonPath) {
...
exports.CORRELATION_PLACEHOLDER_RE = /__CORR_([A-Za-z0-9]+)_(\d+)__/g;
...
function applyIdCorrelation(testCases, responseBodies) {
    let tokenCounter = 0;
    for (let i = 0; i < testCases.length; i++) {
        if (testCases[i].method.toUpperCase() === 'GET')
            continue; // only mutating calls create new resources worth correlating
        ...
        for (const cand of collectIdCandidates(body)) {
            ...
            for (let j = i + 1; j < testCases.length; j++) {
                ...
                if (tc.payload)
                    payloadResult = substituteLiteral(tc.payload, cand.value, placeholder);
                if (urlResult.count > 0 || (payloadResult && payloadResult.count > 0)) {
                    tc.url = urlResult.text;
                    if (payloadResult)
                        tc.payload = payloadResult.text;
                    matched = true;
                }
            }
            if (matched) {
                const producer = testCases[i];
                (producer.producesVars ?? (producer.producesVars = [])).push({ token, jsonPath: cand.jsonPath });
            }
        }
    }
}
...
function extractCookieNames(cookieHeaderValue) {
    return cookieHeaderValue
        .split(';')
        .map(part => part.split('=')[0]?.trim())
        .filter((name) => !!name);
}
function parseHar(content, filename = 'file', opts = {}) {
    ...
    for (const entry of entries) {
        ...
        if (req.postData?.text) {
            ...
            if (mime.includes('json')) {
                ...
                try {
                    const fields = {};
                    for (const [k, v] of new URLSearchParams(req.postData.text))
                        fields[k] = v;
        ...
        if (respContent?.text && (respContent.mimeType ?? '').toLowerCase().includes('json')) {
            try {
                responseBody = JSON.parse(respContent.text);
            }
            catch { /* not JSON — no candidates */ }
        }
