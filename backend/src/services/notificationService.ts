...
function readConfigs(): NotificationConfig[] {
  try {
...
async function sendTeams(webhookUrl: string, payload: object): Promise<{ ok: boolean; message: string }> {
  try {
    ...
    if (!res.ok) return { ok: false, message: `Teams webhook returned ${res.status}` };
...
function row(label: string, value: string, bg = '#f8fafc'): string {
...
function buildEmailHtml(exec: ExecutionSummary): string {
...
function buildTeamsCard(exec: ExecutionSummary): object {
...
function buildTeamsText(exec: ExecutionSummary): string {
  const icon = exec.status === 'pass' ? '✅' : exec.status === 'fail' ? '❌' : '⚠️';
...
export const notificationService = {
  ...
  createConfig: (config: NotificationConfig) => {
  ...
  updateConfig: (id: string, updates: Partial<NotificationConfig>) => {
    const all = readConfigs();
  ...
  sendTest: async (id: string) => {
    ...
    const dummyExec: ExecutionSummary = {
      id: 'test-000',
    ...
    return notificationService.dispatch(config, dummyExec);
  },

  dispatch: async (config: NotificationConfig, exec: ExecutionSummary) => {
    if (!config.enabled) return { ok: false, message: 'Config disabled' };
  ...
  dispatchForExecution: async (exec: ExecutionSummary) => {
    const configs = readConfigs().filter(c => c.enabled);
    const results = await Promise.all(configs.map(c => notificationService.dispatch(c, exec)));
    return results;
  },
};
