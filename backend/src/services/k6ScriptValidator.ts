...
export function isBraceBalanced(code: string): boolean {
  const stack: Array<'template' | 'brace'> = [];
  let i = 0;
  const n = code.length;
  ...
  while (i < n) {
    ...
    if (!inTemplate) {
      ...
      if (c === '}') {
        if (stack[stack.length - 1] !== 'brace') return false; // stray close brace
        stack.pop();
        i++;
        continue;
      }
    ...
    if (c === '$' && code[i + 1] === '{') { stack.push('brace'); i += 2; continue; }
    i++;
  }
